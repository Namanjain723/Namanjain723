<div align="center">

![Naman Jain - AI Products & Automation](assets/banner.png)

### Data & AI Analyst · Founder of **PixlForge Studio** · Rewari, India 🇮🇳
**I build production-grade AI agents, LLM automation and BI systems for real businesses - 10+ shipped, deployed and earning revenue.**

[![Portfolio](https://img.shields.io/badge/🌐_Portfolio-pixlforgestudio.in-7C6CFF?style=for-the-badge)](https://pixlforgestudio.in/)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Naman_Jain-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/naman-jain-a41893266)
[![Email](https://img.shields.io/badge/Email-info@pixlforgestudio.in-2DD4BF?style=for-the-badge&logo=gmail&logoColor=white)](mailto:info@pixlforgestudio.in)
[![Instagram](https://img.shields.io/badge/Instagram-@namanjan723-E4405F?style=for-the-badge&logo=instagram&logoColor=white)](https://instagram.com/namanjan723)

</div>

---

## 👨‍💻 About me

- 🤖 **Founder of PixlForge Studio** - building AI products that replace spreadsheets, reduce manual work, and create recurring revenue for real businesses.
- 🏢 **Data & AI Analyst @ Gupta Creations Pvt. Ltd.** - built the Business Command Center running a 185-employee, 7-location retail chain, plus the AI voice agent that handles live customer calls.
- 📊 **Data Analyst Intern @ Nishtya Infotech** - EDA, KPI reporting and SQL analysis on real business data.
- 🧠 **Claude API, agent architecture and RAG** - tool-use routing, multi-agent pipelines and voice AI orchestration running in production, not demos.
- 🎓 **B.Tech CS + Data Science** @ The NorthCap University, Gurugram (2022-2026).
- 📈 **Growing into Data Science** - I pair real domain knowledge (retail ops, finance, product analytics) with ML, deep learning and AI engineering. Every product I ship creates more real data to learn from.
- 🚀 **Currently building:** a portfolio of AI agents, each aimed at a different B2B niche and each earning recurring revenue on its own.

---

## 📈 By the numbers

<div align="center">

| 🤖 | 👥 | 🏬 | 📄 |
|:--:|:--:|:--:|:--:|
| **10** | **185** | **7** | **1** |
| AI products shipped | employees running on a system I built | store locations live in production | research paper published on Zenodo |

</div>

## 🏆 Flagship Products

### 🎛 Business Command Center
> Real-time ops dashboard for a 185-employee, 7-location retail company - built, delivered, live in production.

One screen replaces HR tracker, task manager, performance tool, store-monitoring app and 8 linked FMS systems. MIS auto-grades every employee A+→F weekly/monthly/yearly. Live alteration pipeline. Task delegation with on-time/late scoring. Voice assistant. Runs on Google Apps Script + Sheets - no servers, no monthly SaaS fees. Owned outright.

**Built for:** Gupta Creations Pvt. Ltd.
**Stack:** Google Apps Script · Google Sheets · Chart.js · Web Speech API
🔗 **[View repo →](https://github.com/Namanjain723/pixlforge-command-center)**

<table>
<tr>
<td width="50%"><img src="https://raw.githubusercontent.com/Namanjain723/pixlforge-command-center/main/assets/screenshot_01_landing_dark.png" alt="Command Center Dark"></td>
<td width="50%"><img src="https://raw.githubusercontent.com/Namanjain723/pixlforge-command-center/main/assets/screenshot_03_mis_scoring.png" alt="MIS Scoring"></td>
</tr>
</table>

---

### 📊 IntelliSheets AI
> Self-hosted agentic data studio - drop a messy spreadsheet, get clean data, dashboards (including interactive 3D), 90-day forecasts, and a chat-with-your-data AI analyst. 100% local, GDPR-compliant. Tally XML import is something no other tool offers.

**Stack:** Python · Streamlit · Plotly · Claude (BYOK) · 100% local
🔗 **[View repo →](https://github.com/Namanjain723/pixlforge-intellisheets)**

<table>
<tr>
<td width="50%"><img src="https://raw.githubusercontent.com/Namanjain723/pixlforge-intellisheets/main/assets/01_dashboard.png" alt="Auto dashboard"></td>
<td width="50%"><img src="https://raw.githubusercontent.com/Namanjain723/pixlforge-intellisheets/main/assets/03_3d.png" alt="3D explorer"></td>
</tr>
</table>

---

## 🤖 All AI Products

<table>
<tr>
<td width="50%" valign="top">
  <a href="https://github.com/Namanjain723/pixlforge-command-center"><img src="https://raw.githubusercontent.com/Namanjain723/pixlforge-command-center/main/assets/thumbnail_main.png" alt="Business Command Center"/></a>
  <br><b>🎛 Business Command Center</b>
  <br><sub>Real-time ops dashboard for multi-location retail - MIS scoring, live alteration pipeline, task delegation, voice assistant. <em>Apps Script · Sheets · JS</em></sub>
  <br>🔗 <a href="https://github.com/Namanjain723/pixlforge-command-center"><b>View repo →</b></a>
</td>
<td width="50%" valign="top">
  <a href="https://github.com/Namanjain723/pixlforge-intellisheets"><img src="https://raw.githubusercontent.com/Namanjain723/pixlforge-intellisheets/main/assets/00_hero.png" alt="IntelliSheets AI"/></a>
  <br><b>📊 IntelliSheets AI</b>
  <br><sub>Self-hosted agentic data studio - auto-clean, dashboard (incl. 3D), AI analyst, forecasting, Tally XML import. <em>Python · Streamlit · Plotly · Claude</em></sub>
  <br>🔗 <a href="https://github.com/Namanjain723/pixlforge-intellisheets"><b>View repo →</b></a>
</td>
</tr>
<tr>
<td width="50%" valign="top">
  <a href="https://github.com/Namanjain723/pixlforge-pulse"><img src="https://raw.githubusercontent.com/Namanjain723/pixlforge-pulse/main/assets/thumbnail_main.png" alt="PixlForge Pulse"/></a>
  <br><b>📣 PixlForge Pulse</b>
  <br><sub>Autonomous AI marketing-reporting agent - turns raw data into branded client reports every Monday, unattended. White-label ready. <em>n8n · Claude</em></sub>
  <br>🔗 <a href="https://github.com/Namanjain723/pixlforge-pulse"><b>View repo →</b></a>
</td>
<td width="50%" valign="top">
  <a href="https://github.com/Namanjain723/pixlforge-competitor-monitor"><img src="https://raw.githubusercontent.com/Namanjain723/pixlforge-competitor-monitor/main/assets/hero.png" alt="Competitor Monitor"/></a>
  <br><b>🔭 Competitor Monitor</b>
  <br><sub>24/7 AI competitive intelligence for Shopify - 10 change types, 0-100 Opportunity Score, Claude Haiku Action Briefs. ~$0.12/client/month. <em>Python · DuckDB · Claude Haiku 4.5</em></sub>
  <br>🔗 <a href="https://github.com/Namanjain723/pixlforge-competitor-monitor"><b>View repo →</b></a>
</td>
</tr>
<tr>
<td width="50%" valign="top">
  <a href="https://github.com/Namanjain723/pixlforge-sentinel"><img src="https://raw.githubusercontent.com/Namanjain723/pixlforge-sentinel/main/assets/hero.png" alt="Sentinel"/></a>
  <br><b>🛰 Sentinel</b>
  <br><sub>Budget-gated B2B lead intelligence - Google Places + Playwright + Claude Haiku 4.5, hard budget cap, honest SMTP verification, CSV + branded PDF. <em>Python · SQLite</em></sub>
  <br>🔗 <a href="https://github.com/Namanjain723/pixlforge-sentinel"><b>View repo →</b></a>
</td>
<td width="50%" valign="top">
  <a href="https://github.com/Namanjain723/pixlforge-autobooks-ai"><img src="https://raw.githubusercontent.com/Namanjain723/pixlforge-autobooks-ai/main/assets/thumbnail_main.png" alt="AutoBooks AI"/></a>
  <br><b>🧾 AutoBooks AI</b>
  <br><sub>Autonomous bookkeeping agent for accounting firms - reads invoices, writes weekly AI reports, detects anomalies, scores tax-readiness. <em>n8n · Claude · Streamlit</em></sub>
  <br>🔗 <a href="https://github.com/Namanjain723/pixlforge-autobooks-ai"><b>View repo →</b></a>
</td>
</tr>
<tr>
<td width="50%" valign="top">
  <a href="https://github.com/Namanjain723/pixlforge-concierge"><img src="https://raw.githubusercontent.com/Namanjain723/pixlforge-concierge/main/assets/hero.png" alt="Concierge"/></a>
  <br><b>💬 Concierge</b>
  <br><sub>24/7 WhatsApp booking agent for clinics, salons & studios - books appointments, kills no-shows, captures every lead. White-label, multi-tenant. <em>Claude · WhatsApp API · FastAPI</em></sub>
  <br>🔗 <a href="https://github.com/Namanjain723/pixlforge-concierge"><b>View repo →</b></a>
</td>
<td width="50%" valign="top">
  <a href="https://github.com/Namanjain723/pixlforge-frontdesk"><img src="https://raw.githubusercontent.com/Namanjain723/pixlforge-frontdesk/main/assets/hero.png" alt="Frontdesk AI"/></a>
  <br><b>📞 Frontdesk AI</b>
  <br><sub>AI phone receptionist - answers every call on the first ring, books appointments, handles FAQs, zero missed calls. <em>Claude · Plivo · Deepgram · Azure TTS</em></sub>
  <br>🔗 <a href="https://github.com/Namanjain723/pixlforge-frontdesk"><b>View repo →</b></a>
</td>
</tr>
<tr>
<td width="50%" valign="top">
  <a href="https://github.com/Namanjain723/pixlforge-aegis"><img src="https://raw.githubusercontent.com/Namanjain723/pixlforge-aegis/main/assets/hero.png" alt="Aegis"/></a>
  <br><b>🛡 Aegis</b>
  <br><sub>Compliance-first reputation defense - AI detects coordinated review attacks, 3-tier Action Engine with code-enforced guardrails, Evidence Builder, Response Studio. <em>FastAPI · Next.js · Claude</em></sub>
  <br>🔗 <a href="https://github.com/Namanjain723/pixlforge-aegis"><b>View repo →</b></a>
</td>
<td width="50%" valign="top">
  <a href="https://github.com/Namanjain723/pixlforge-intellect-seo"><img src="https://raw.githubusercontent.com/Namanjain723/pixlforge-intellect-seo/main/assets/hero.png" alt="Intellect SEO"/></a>
  <br><b>🧠 Intellect SEO Estimator</b>
  <br><sub>AI schema.org JSON-LD generator for WordPress - hard budget cap, always-on safe fallback, white-label PDF reports, Ed25519 licensing. <em>PHP 8 · Claude · WordPress</em></sub>
  <br>🔗 <a href="https://github.com/Namanjain723/pixlforge-intellect-seo"><b>View repo →</b></a>
</td>
</tr>
</table>

---

## 📊 Data Science & Analytics

My long-term direction is **Data Science** - I work where domain knowledge (retail ops, finance, marketing), applied ML and production AI systems meet. Every product I ship produces real business data; the models I'm learning now will process it.

**Published research:**

> 📄 **Business Performance Optimization Through Sales Data Analysis**
> *Published on Zenodo · DOI: [10.5281/zenodo.18069636](https://doi.org/10.5281/zenodo.18069636)*
> EDA + multi-KPI analysis on real retail sales data. Actionable findings presented to business stakeholders.

**Projects:**

<table>
<tr>
<td width="50%" valign="top">
  <b>₿ Bitcoin Sentiment Trader Analysis</b>
  <br><sub>Does the Crypto Fear & Greed Index actually predict trader profitability? Analyzed real trade-level data to find out - EDA, correlation analysis, risk-return breakdown by sentiment regime.</sub>
  <br><sub><em>Python · Pandas · Matplotlib · Seaborn · EDA</em></sub>
  <br>🔗 <a href="https://github.com/Namanjain723/Bitcoin-Sentiment-Trader-Analysis">View repo →</a>
</td>
<td width="50%" valign="top">
  <b>📈 Stock Price Prediction (LSTM)</b>
  <br><sub>Deep-learning time-series model forecasting Netflix stock prices. Multi-step LSTM with train/val/test split, rolling window, RMSE benchmarked against baseline.</sub>
  <br><sub><em>Python · TensorFlow · Keras · LSTM · NumPy</em></sub>
  <br>🔗 <a href="https://github.com/Namanjain723/stock-price-prediction-lstm">View repo →</a>
</td>
</tr>
<tr>
<td width="50%" valign="top">
  <b>📊 Sales Performance & Customer Insights</b>
  <br><sub>Full BI solution on AdventureWorks - multi-page Power BI report covering revenue, customer segments, product performance, returns. Complex DAX measures + SQL Server backend.</sub>
  <br><sub><em>SQL Server · Power BI · DAX · Data Modeling</em></sub>
  <br>🔗 <a href="https://github.com/Namanjain723/sales-insights-dashboard">View repo →</a>
</td>
<td width="50%" valign="top">
  <b>🧪 A/B Testing Analysis</b>
  <br><sub>Rigorous two-sample t-test on a real experiment - normality checks, variance testing, effect size (Cohen's d), confidence intervals, business interpretation of results.</sub>
  <br><sub><em>Python · SciPy · Pandas · Statistical Inference</em></sub>
  <br>🔗 <a href="https://github.com/Namanjain723/ab-test-analysis">View repo →</a>
</td>
</tr>
<tr>
<td width="50%" valign="top">
  <b>🚢 Titanic Survival Classification</b>
  <br><sub>End-to-end ML pipeline - feature engineering, missing-value imputation, model comparison (Logistic Regression, Random Forest, SVM), cross-validated accuracy.</sub>
  <br><sub><em>Python · scikit-learn · Matplotlib · EDA</em></sub>
  <br>🔗 <a href="https://github.com/Namanjain723/Titanic-classification">View repo →</a>
</td>
<td width="50%" valign="top">
  <b>📐 What's next</b>
  <br><sub>Currently deepening: time-series forecasting at production scale, NLP for unstructured business data, and ML pipelines that feed back into the AI products I'm shipping. These repos will appear here as they're built.</sub>
</td>
</tr>
</table>

---

## 🧰 Tech stack

**Languages & Querying**
![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat-square&logo=mysql&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)
![PHP](https://img.shields.io/badge/PHP-777BB4?style=flat-square&logo=php&logoColor=white)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white)

**AI & Automation**
![Anthropic Claude](https://img.shields.io/badge/Claude-D97757?style=flat-square&logo=anthropic&logoColor=white)
![n8n](https://img.shields.io/badge/n8n-EA4B71?style=flat-square&logo=n8n&logoColor=white)
![FastAPI](https://img.shields.io/badge/FastAPI-009688?style=flat-square&logo=fastapi&logoColor=white)
![Next.js](https://img.shields.io/badge/Next.js-000000?style=flat-square&logo=nextdotjs&logoColor=white)

**Data Science & ML**
![Pandas](https://img.shields.io/badge/Pandas-150458?style=flat-square&logo=pandas&logoColor=white)
![NumPy](https://img.shields.io/badge/NumPy-013243?style=flat-square&logo=numpy&logoColor=white)
![scikit-learn](https://img.shields.io/badge/scikit--learn-F7931E?style=flat-square&logo=scikitlearn&logoColor=white)
![TensorFlow](https://img.shields.io/badge/TensorFlow-FF6F00?style=flat-square&logo=tensorflow&logoColor=white)
![Matplotlib](https://img.shields.io/badge/Matplotlib-11557C?style=flat-square&logo=python&logoColor=white)
![Seaborn](https://img.shields.io/badge/Seaborn-4C8CBF?style=flat-square&logo=python&logoColor=white)
![SciPy](https://img.shields.io/badge/SciPy-8CAAE6?style=flat-square&logo=scipy&logoColor=white)

**Visualization & BI**
![Streamlit](https://img.shields.io/badge/Streamlit-FF4B4B?style=flat-square&logo=streamlit&logoColor=white)
![Plotly](https://img.shields.io/badge/Plotly-3F4F75?style=flat-square&logo=plotly&logoColor=white)
![Power BI](https://img.shields.io/badge/Power_BI-F2C811?style=flat-square&logo=powerbi&logoColor=black)
![Google Apps Script](https://img.shields.io/badge/Apps_Script-4285F4?style=flat-square&logo=google&logoColor=white)

---

---

## 📊 GitHub activity

<div align="center">

<img height="180" alt="Naman Jain's GitHub profile summary" src="https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=Namanjain723&theme=tokyonight" />

<img height="165" alt="Repositories per language" src="https://github-profile-summary-cards.vercel.app/api/cards/repos-per-language?username=Namanjain723&theme=tokyonight" />
<img height="165" alt="Most used language by commits" src="https://github-profile-summary-cards.vercel.app/api/cards/most-commit-language?username=Namanjain723&theme=tokyonight" />

<img height="165" alt="Commit statistics" src="https://github-profile-summary-cards.vercel.app/api/cards/stats?username=Namanjain723&theme=tokyonight" />
<img height="165" alt="Most productive hours" src="https://github-profile-summary-cards.vercel.app/api/cards/productive-time?username=Namanjain723&utcOffset=5.5&theme=tokyonight" />

<img width="98%" alt="Contribution graph" src="https://ghchart.rshah.org/7C6CFF/Namanjain723" />

</div>

## 💼 Experience

**Data & AI Analyst** - *Gupta Creations Pvt. Ltd., Delhi* · Apr 2026 - Present
- Built a real-time Command Center consolidating 8 management systems for a 185-employee, 7-location retail chain; automated performance scoring and task delegation, removing manual tracking entirely.
- Built a custom AI voice agent for live customer calls - reads Google Sheets in real time to resolve alteration, discount and service queries autonomously at CRM level, not IVR.
- Designed the ETL pipelines and LLM workflows behind both, cutting SaaS spend by ₹10,000/month.

**AI & Data Consultant** - *PixlForge Studio, Rewari* · Jan 2026 - Apr 2026
- MSME registered (Udyam, Govt. of India). Delivered 10+ production BI dashboards and AI automation systems for SMB clients across healthcare, retail and SaaS, each closed with measurable cost savings.
- Launched Frontdesk AI (voice receptionist, live in 5+ Kamla Nagar, Delhi salons at ₹3,999/month), Sentinel (5-agent autonomous lead pipeline) and IntelliSheets (agentic data studio) - all commercially deployed with recurring revenue.

**Data Analyst Intern** - *Nishtya Infotech Pvt. Ltd., Gurugram* · Jun 2025 - Mar 2026
- Cleaned and analyzed 10,000+ records with SQL, Python (Pandas, NumPy) and advanced Excel; applied EDA and hypothesis testing to surface KPI anomalies that informed weekly business decisions.
- Built Power BI dashboards with DAX and Power Query for cross-functional reporting, reducing ad hoc analytics requests organization-wide.

## 🎓 Education

**B.Tech in Computer Science - Specialization in Data Science**
🏫 *The NorthCap University, Gurugram* · 📅 *Sept 2022 - Mar 2026*

## 📜 Certifications

- **Jobaaj Learning** - Data & Business Analytics Specialization, Nano Degree (2026)
- **Physics Wallah** - Data Science with Generative AI (2025)
- **Microsoft** - Azure Internship & Training (2025)
- **Microsoft & NASBA** - Learning Data Analytics, Part 2

---

<div align="center">

### 📫 Want a demo or want to work together?

**I'm open to:** Data & AI analyst roles · AI automation builds · Data analysis projects · Freelance engagements · Full-time roles (2026+)

[![Portfolio](https://img.shields.io/badge/Portfolio-pixlforgestudio.in-7C6CFF?style=for-the-badge&logo=googlechrome&logoColor=white)](https://pixlforgestudio.in/)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-naman--jain-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/naman-jain-a41893266)
[![Email](https://img.shields.io/badge/Email-info@pixlforgestudio.in-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:info@pixlforgestudio.in)
[![Direct](https://img.shields.io/badge/Direct-namancric18@gmail.com-6B7280?style=for-the-badge&logo=maildotru&logoColor=white)](mailto:namancric18@gmail.com)

*Building AI + Data Science at the same time - because the best models run on real business data.* ⚡ **Naman Jain** · PixlForge Studio

</div>
